...
import { FiX, FiEdit2, FiTrash2, FiTarget } from 'react-icons/fi';

export default function Zone({ zones, upazilas, pagination }) {
        ...
        <AppLayout>
            ...
            <div className="p-3 sm:p-5">
                <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mb-6 bg-secondary p-4 rounded-lg shadow">
                    <h1 className="text-xl sm:text-2xl font-bold text-primary-content">🎯 Zones</h1>
                    <button
                            ...
                            <div className="bg-gradient-to-r from-primary to-primary-hover p-4 text-white">
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 pr-2">
                                        <h3 className="text-lg sm:text-xl font-bold mb-1">{zone.name}</h3>
                                        <span className="text-xs bg-white/20 px-2 py-1 rounded font-semibold inline-block">
                                            {zone.code}
                                        </span>
                                    </div>
                                    <FiTarget size={24} className="opacity-80" />
                                </div>
                            </div>
                                ...
                                <div className="space-y-2">
                                    {zone.upazila && (
                                        <>
                                            <div className="flex items-center justify-between text-sm">
                                                <span className="text-primary-content/60">Upazila:</span>
                                    ...
                                    <div className="flex items-center justify-between text-sm pt-2 border-t border-accent">
                                        ...
                                        <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                            zone.is_active ? 'bg-success/20 text-success' : 'bg-error/20 text-error'
                                        }`}>
                ...
                {(!zones?.data || zones.data.length === 0) && (
                    <div className="text-center py-16 bg-base-200 rounded-lg">
                        ...
                        <h3 className="text-xl font-semibold text-primary-content mb-2">No Zones Yet</h3>
                        <p className="text-primary-content/60 mb-4">Get started by creating your first zone</p>
                        <button
                            onClick={() => setIsOpen(true)}
                    ...
                    <div className="bg-base-200 rounded-xl max-w-2xl w-full max-h-[95vh] overflow-hidden shadow-2xl animate-fadeIn">
                        ...
                        <div className="bg-gradient-to-r from-primary to-primary-hover p-4 sm:p-5 flex items-center justify-between">
                            <h2 className="text-lg sm:text-xl font-bold text-white">
                                {isEditMode ? '✏️ Edit Zone' : '➕ Create New Zone'}
                            </h2>
                            <button
                                onClick={closeModal}
                        ...
                        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 overflow-y-auto max-h-[calc(95vh-180px)]">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm sm:text-base font-semibold mb-2 text-primary-content">
                                        Zone Name <span className="text-error">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        value={data.name}
                                ...
                                <div>
                                    <label className="block text-sm sm:text-base font-semibold mb-2 text-primary-content">
                                        Zone Code <span className="text-error">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        value={data.code}
                                ...
                                >
                                    ...
                                    {upazilas?.map((upazila) => (
                                        <option key={upazila.id} value={upazila.id}>
                                            {upazila.name} ({upazila.code}) - {upazila.district?.name}
                                        </option>
                                    ))}
                                </select>
                                {errors.upazila_id && <p className="text-error text-sm mt-1">{errors.upazila_id}</p>}
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-accent">
                                ...
                                >
                                    {processing ? 'Saving...' : isEditMode ? 'Update Zone' : 'Create Zone'}
                                </button>
                            </div>
                        </form>
                    </div>
